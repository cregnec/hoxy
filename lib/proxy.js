...
export default class Proxy extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    let createServer = opts.tls ? https.createServer.bind(https, opts.tls) : http.createServer.bind(http);
    ...
    this._server = createServer((fromClient, toClient) => {
      ...
      co.call(this, function* () {
        ...
        try {
          yield this._runIntercepts('request', cycle);
        } catch (ex) {
          this._emitError(ex, 'request');
        }
        let partiallyFulfilledRequest = yield cycle._sendToServer();
        try {
          yield this._runIntercepts('request-sent', cycle);
        } catch (ex) {
          this._emitError(ex, 'request-sent');
        }
  ...
  log(events, cb) {
    let listenTo = {};
    events.split(/\s/).map(s => s.trim()).filter(s => !!s).forEach(s => listenTo[s] = true);
    let writable;
    if (!cb) {
